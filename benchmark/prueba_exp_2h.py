...
def f_action(u_actual,p,pd_hunt):
    ...
    solX = np.transpose(Q) + np.transpose(Qd) + M*np.transpose(np.matrix([u_actual[0],u_actual[1],u_actual[2],u_actual[3]])) + 0.1*(np.transpose(p) - np.transpose(pd_hunt)) 
...
while (k<5000):
    ...
    X2 = (p[0,0]-h2[0,0])**2+(p[0,1]-h2[0,1])**2
    if norm(p-h1)<1.0 or norm(p-h2)<1.0:
        pdot = 1.5*(p-h1)*np.exp(-(1/(1**2))*X1) + 1.5*(p-h2)*np.exp(-(1/(1**2))*X2)
    else:
        pdot = 0.5*1.5*(p-h1)*np.exp(-(1/(1**2))*X1) + 0.5*1.5*(p-h2)*np.exp(-(1/(1**2))*X2)
