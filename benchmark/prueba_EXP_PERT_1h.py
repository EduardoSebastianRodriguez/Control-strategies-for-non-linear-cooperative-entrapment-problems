...
def f_action(u_actual,p,pd_vaca,pd_hunt):
    ...
    if (norm(p-u_actual)<1.0):
         ...
         Q = 1.5*0.5*(p)*np.exp(-(1/(1**2))*X)
         M = -1.5*0.5*np.exp(-(1/(1**2))*X)
         
    solX = M*u_actual+Q+10*(pd_vaca-pd_hunt)+W*(np.transpose(p)+np.transpose(pd_hunt))
...
while (k<K):
    ...
    if norm(p-h)<1.0:
        pdot = 1.5*(p-h)*np.exp(-(1/(1**2))*X) + (pd_vaca-p) + np.matrix([W[0,0]*p[0,0]+W[0,1]*p[0,1],W[1,0]*p[0,0]+W[1,1]*p[0,1]])
    else:
        pdot = 1.5*0.5*(p-h)*np.exp(-(1/(1**2))*X) + (pd_vaca-p) + np.matrix([W[0,0]*p[0,0]+W[0,1]*p[0,1],W[1,0]*p[0,0]+W[1,1]*p[0,1]])
